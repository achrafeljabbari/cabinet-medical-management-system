...
Route::post('/doctors/register', [DoctorController::class, 'register'])->name('api.doctors.register');
Route::post('/admins/register', [AdminController::class, 'register'])->name('api.admins.register');
Route::post('/secretaries/register', [SecretaryController::class, 'register'])->name('api.secretaries.register');
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::get('/patients/{id}/appointments', [PatientController::class, 'getAppointments'])
        ->name('api.patients.appointments');
    ...
    Route::get('/patients/{id}/medical-info', [PatientController::class, 'getMedicalInfo'])
        ->name('api.patients.medical-info');
    ...
    Route::get('/appointments/doctor/{doctor_id}', [AppointmentController::class, 'getByDoctor'])
    ...
    Route::get('/consultations/patient/{patient_id}', [ConsultationController::class, 'getByPatient'])
        ->name('api.consultations.patient');

    // ==================== ORDONNANCES ====================
    Route::apiResource('ordonnances', OrdonnanceController::class)->names('api.ordonnances');

    Route::get('/consultations/{consultation_id}/ordonnances', [OrdonnanceController::class, 'getByConsultation'])
        ->name('api.ordonnances.consultation');

    // ==================== DOSSIER MEDICAL ====================
    Route::apiResource('dossiers-medicaux', DossierMedicalController::class)->names('api.dossiers');

    Route::get('/dossiers-medicaux/patient/{patient_id}', [DossierMedicalController::class, 'getByPatient'])
        ->name('api.dossiers.patient');

    Route::get('/dossiers-medicaux/patient/{patient_id}/summary', [DossierMedicalController::class, 'getSummary'])
        ->name('api.dossiers.summary');
    ...
    Route::get('/cabinets/{id}/doctors', [CabinetController::class, 'getDoctors'])
        ->name('api.cabinets.doctors');

    Route::get('/cabinets/search/name', [CabinetController::class, 'searchByName'])
        ->name('api.cabinets.search');
